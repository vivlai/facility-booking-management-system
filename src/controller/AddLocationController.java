...
public class AddLocationController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String stringLocationName = request.getParameter("location");
		Location location = LocationDao.getInstance().getLocation(stringLocationName);
		if (location != null) {
			response.sendRedirect("admin-add-location.jsp?error=Location has already existed");
			return;
		}
		
		Location newLocation = new Location(/*autogenerated*/ -1, stringLocationName);
		LocationDao.getInstance().addLocation(newLocation);
		response.sendRedirect("admin-view-location.jsp?message=Location has been added");
	}
}
